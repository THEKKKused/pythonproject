...
class CalibrationWorker(QThread):
    ...
    def _write_stage_all_ok(
            self,
            rt_points_by_port: Dict[str, List[Tuple[int, float]]],
            ad_points_by_port: Dict[str, List[Tuple[int, float]]],
    ) -> Tuple[bool, List[str]]:
        """检查写入阶段是否已完成（Rt/AD写入+回读）。返回(ok, 失败列表)."""
        fails: List[str] = []
        for d in self.devices:
            sn_key = sanitize_sn(d.current_sn or b'')
            info = self.report.get(sn_key, {}) if isinstance(self.report, dict) else {}

            rt_expected = len(rt_points_by_port.get(d.port, []) or []) == 3
            ad_expected = len(ad_points_by_port.get(d.port, []) or []) == 3

            rt_ok = bool(info.get('rt_write_ok') and info.get('rt_readback'))
            ad_ok = bool(info.get('ad_write_ok') and info.get('ad_readback'))

            reasons: List[str] = []
            if not rt_expected:
                reasons.append('RT点不足(缺E)')
            elif not rt_ok:
                reasons.append('RT写入/回读失败')

            if not ad_expected:
                reasons.append('AD点不足')
            elif not ad_ok:
                reasons.append('AD写入/回读失败')

            if reasons:
                fails.append(f"{d.port} SN={sn_key}: " + '; '.join(reasons))
    ...
    def run(self):
        ...
        try:
            ...
            # 7.3) 写入失败保护：若存在写入/回读失败，不要直接进入60℃复核（避免“未写入却开始复核”）
            if self.do_60c_verify and not self._stop.is_set():
                ok_all, fail_lines = self._write_stage_all_ok(rt_points_by_port, ad_points_by_port)
                if not ok_all:
                    self.log_signal.emit('❗ 检测到写入/回读未完成，已暂停进入60℃复核。请检查一拖8板/接触后点击“接触不良，重新扫描”（将恢复出厂并用已保存三点重写）。')
                    for ln in fail_lines[:30]:
                        self.log_signal.emit('  - ' + ln)
                    if len(fail_lines) > 30:
                        self.log_signal.emit(f'  ... 共 {len(fail_lines)} 个传感器未完成写入/回读')
                    self.stage_signal.emit('写入失败：暂停60℃复核（等待“接触不良，重新扫描”或“停止”）')

                    # 等待用户处理（点击“接触不良”会触发_rescan，然后执行恢复出厂+重写）
                    while not self._stop.is_set():
                        if self._rescan.is_set():
                            self.stage_signal.emit('写入失败：执行接触不良处理（恢复出厂+重写）...')
                            if self._recover_write_stage_after_bad_contact(rt_points_by_port, ad_points_by_port):
                                ok_all2, fail_lines2 = self._write_stage_all_ok(rt_points_by_port, ad_points_by_port)
                                if ok_all2:
                                    self.log_signal.emit('✅ 重写后写入/回读已全部成功，将继续60℃复核。')
                                    break
                                else:
                                    self.log_signal.emit('⚠️ 重写后仍有未完成写入/回读，继续等待处理...')
                                    for ln in fail_lines2[:30]:
                                        self.log_signal.emit('  - ' + ln)
                                    if len(fail_lines2) > 30:
                                        self.log_signal.emit(f'  ... 共 {len(fail_lines2)} 个')
                        time.sleep(0.3)
...
class MainWindow(QMainWindow):
    ...
    def _build_tab_cal(self) -> QWidget:
        ...
        # IoTDB
        gb_i = QGroupBox('IoTDB（需要保证IoTDB服务已启动；软件负责连接和入库）')
        il = QGridLayout()
